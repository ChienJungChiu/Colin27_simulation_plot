...
%}
%tissue_param,vol,voxel_size,p_pos,p_dir,sim_set,subject_folder,output_fluence_rate,output_pathlength,output_jacobian,1,0,0
...
num_shuffle_time=10; % if check CV, do this times to average the CV
...
%% initialize
global sim_version;
assert(sim_version==4.51,'ERROR: The version of main and function is not consistent!');
...
elseif strcmp(source_type,'disk')
    ...
    cfg.srcparam2=[0 0 0 0];
elseif strcmp(source_type,'pattern')
    cfg.srctype='pattern';
    cfg.srcpattern=sim_set.srcpattern;
    cfg.srcnum=1;
    cfg.srcparam1=sim_set.srcparam1;
    cfg.srcparam2=sim_set.srcparam2;
    cfg.srcparam1([1 2])=cfg.srcparam1([2 1]); % exchange the x, y pos
    cfg.srcparam2([1 2])=cfg.srcparam2([2 1]); % exchange the x, y pos
    
%     cfg.srctype='planar';
elseif strcmp(source_type,'zgaussian')
    cfg.srctype='zgaussian';
    cfg.srcparam1=sim_set.srcparam1;
    cfg.srcparam2=sim_set.srcparam2;
elseif strcmp(source_type,'anglepattern')
...
if gpuDeviceCount==1
    ...
    CUDA_dev_info=gpuDevice;
    sim_summary.sim_GPU=CUDA_dev_info.Name;
else
    GPU_setting=load('GPU_setting.txt');
    assert(sum(GPU_setting(1,:))>=1,'ERROR: No GPU selection!');
    assert(gpuDeviceCount>=max(find(GPU_setting(1,:)==1)),'ERROR: Select GPU not exist!');
    if sum(GPU_setting(1,:))==1
        cfg.gpuid=find(GPU_setting(1,:)==1);
        CUDA_dev_info=gpuDevice(cfg.gpuid);
        sim_summary.sim_GPU=CUDA_dev_info.Name;
    else
        gpuid_arr='';
        workload_arr=[];
        for gp=1:gpuDeviceCount
            gpuid_arr(1,gp)=num2str(GPU_setting(1,gp),'%d');
            workload_arr(1,gp)=GPU_setting(2,gp);
            if GPU_setting(1,gp)==1
                CUDA_dev_info=gpuDevice(gp);
                sim_summary.sim_GPU=[sim_summary.sim_GPU CUDA_dev_info.Name ','];
            end
        end
        cfg.gpuid=gpuid_arr;
...
cfg.isnormalized=0; % normalize the output fluence to unitary source
...
cfg.isgpuinfo=1; % 1-print GPU info, [0]-do not print
...
cfg.issaveref=1;
cfg.isreflect=1; % consider index mismatch reflect
...
for wl=wl_start_index:size(tissue_param,1)
    ...
    rng('shuffle','twister'); 
    %%I change here because of The current random number generator is the legacy generator.  
    %%This is because you have executed a command such asrand('state',0), which activates MATLAB's legacy random number behavior.  
    ...
    %%Error in fun_MCX_sim_dist2axis (line 248)  rng('shuffle');
    ...
    while run_index<num_need_to_run
        ...
        if show_MCX_info
            [fluence rate,detpt,~,seeds]=mcxlab(cfg);
        else
            ...
            if isfield(detpt,'detid') % to avoid that there are no photon detected in this simulation
                ...
                for s=1:num_SDS
                    ...
                    if length(in_dist_detpt)>0
                        ...
                        if output_pathlength
                            detPL_arr{s}=[detPL_arr{s}; detpt.ppath(in_NA_index,to_output_layer)];
                        end
                        if output_jacobian
        ...
        if output_fluence_rate
            if run_index==1
                fluencerate_arr=double(fluencerate.data)./photon_per_simulation; % normalize to the total photon number
            else
                fluencerate_arr=fluencerate_arr+(double(fluencerate.data)./photon_per_simulation); % normalize to the total photon number
            end
        end
        ...
        if do_checkpoint~=0
            ...
            if output_jacobian && output_pathlength
                save(fullfile(output_folder,['detpt_chk_' num2str(run_index) '.mat']),'seed_arr','detpt_arr','detPL_arr');
            ...
            elseif output_pathlength
                save(fullfile(output_folder,['detpt_chk_' num2str(run_index) '.mat']),'detPL_arr');
            end
    ...
    if output_fluence_rate
        average_fluence_rate=fluencerate_arr./num_need_to_run;
    ...
    if output_pathlength
        ...
        if do_checkpoint~=0
            % init
            SDS_detpt_arr=cell(1,num_SDS);
            for s=1:num_SDS
                SDS_detpt_arr{s}=single(zeros(SDS_detected_numpt(s),length(to_output_layer)));
            end
            % load the checkpoints
            SDS_load_numpt=ones(1,num_SDS);
            for ridx=1:num_need_to_run
                ...
                for s=1:num_SDS
                    ...
                    SDS_detpt_arr{s}(old_SDS_numpt:SDS_load_numpt(s)-1,:)=temp_detPL.detPL_arr{s};
                end
            end
        else
            SDS_detpt_arr=detPL_arr;
        end
        
        % turn voxel length (mm) into cm
        sim_summary.SDS_detected_number=zeros(1,num_SDS);
        for s=1:num_SDS
            SDS_detpt_arr{s}=SDS_detpt_arr{s}./10.*voxel_size;
            sim_summary.SDS_detected_number(1,s)=size(SDS_detpt_arr{s},1);
        end
        
        %% check CV and try to reduce photon number
        if max_CV>0
            fprintf('\tChecking CV: SDS ');
            ...
            for s=1:num_SDS
                fprintf('%d ',s)
                group_CV_arr{s}=[];
                if size(SDS_detpt_arr{s},1)/2>=min_detpt
                    each_photon_detected_weight=exp(-double(sum(SDS_detpt_arr{s}.*tissue_param(wl,(2*to_output_layer)-1),2)));
                    to_divide_group=2;
                    divide_group_okey=1;
                    to_break_flag=0;
                    while size(SDS_detpt_arr{s},1)/to_divide_group>=min_detpt
                        photon_in_subGroup=ones(1,to_divide_group)*floor(size(SDS_detpt_arr{s},1)/to_divide_group);
                        photon_in_subGroup(1:rem(size(SDS_detpt_arr{s},1),to_divide_group))=photon_in_subGroup(1:rem(size(SDS_detpt_arr{s},1),to_divide_group))+1;
                        assert(sum(photon_in_subGroup)==size(SDS_detpt_arr{s},1));
                        photon_in_subGroup=cumsum([0 photon_in_subGroup]);
                        CV_ref_subGroup=zeros(to_divide_group,num_shuffle_time);
                        for sh=1:num_shuffle_time
                            shuffled_index=randperm(size(SDS_detpt_arr{s},1));
                            for sg=1:to_divide_group
                                CV_ref_subGroup(sg,sh)=sum(each_photon_detected_weight(shuffled_index(photon_in_subGroup(sg)+1:photon_in_subGroup(sg+1))));
                            end
                        end
                        CV_ref_subGroup=std(CV_ref_subGroup)./mean(CV_ref_subGroup,1);
                        temp_CV=sqrt(mean(CV_ref_subGroup.^2));
                        group_CV_arr{s}(end+1,:)=[to_divide_group temp_CV];
                        if temp_CV<=max_CV
                            divide_group_okey=to_divide_group;
                            to_divide_group=ceil(to_divide_group*1.2);
                            if size(SDS_detpt_arr{s},1)/to_divide_group<min_detpt
                                to_break_flag=1;
                            end
                        else
                            to_break_flag=1;
                        ...
                        if to_break_flag==1
                            each_photon_weight_arr(s)=num_photon/divide_group_okey;
                            divide_times_arr(s)=divide_group_okey;
                            SDS_detpt_arr{s}=SDS_detpt_arr{s}(shuffled_index(1:floor(size(SDS_detpt_arr{s},1)/divide_group_okey)),:);
                            break;
                        end
                    end
                else
                    ...
                    % SDS_detpt_arr{s}=SDS_detpt_arr{s};
                    group_CV_arr{s}=[1 0];
                end
            end
            fprintf('\n');
            ...
            divide_times_arr=ones(1,num_SDS);
        end
        
        %% save output file
        detPL_state=whos('SDS_detpt_arr');
        if detPL_state.bytes/1024^2>500 % if the file will be too large
            save(fullfile(output_folder,['PL_' num2str(wl) '.mat']),'SDS_detpt_arr','each_photon_weight_arr','divide_times_arr','-v7.3');
        else
            save(fullfile(output_folder,['PL_' num2str(wl) '.mat']),'SDS_detpt_arr','each_photon_weight_arr','divide_times_arr');
            if exist(fullfile(output_folder,['PL_' num2str(wl) '.mat']),'file')==0
                save(fullfile(output_folder,['PL_' num2str(wl) '.mat']),'SDS_detpt_arr','each_photon_weight_arr','divide_times_arr','-v7.3');
            end
        end
        if max_CV>0
            save(fullfile(output_folder,['group_CV_arr_' num2str(wl) '.mat']),'group_CV_arr');
        end
